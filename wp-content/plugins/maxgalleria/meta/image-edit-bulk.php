...
if ($_POST && check_admin_referer($image_gallery->nonce_image_edit_bulk['action'], $image_gallery->nonce_image_edit_bulk['name'])) {
	...
	foreach ($_POST['image-edit-id'] as $image_edit_id) {
		...
		$post_title = $maxgalleria->stripUnwantedTagsAndAttrs($_POST['image-edit-title'][$i]);
    $post_excerpt = $maxgalleria->stripUnwantedTagsAndAttrs($_POST['image-edit-caption'][$i]);
    ...
    $temp['post_excerpt'] = $maxgalleria->remove_ptags($post_excerpt);
		...
		$link = esc_url_raw($_POST['image-edit-link'][$i]);
		if (isset($link) && $link != '') {
			if (!$common->string_starts_with($link, 'http://') && !$common->string_starts_with($link, 'https://')) {
				$link = 'http://' . $link;
			}
		}
		
		// Now update the meta
		update_post_meta($image_edit_id, '_wp_attachment_image_alt', stripslashes(sanitize_text_field($_POST['image-edit-alt'][$i])));
