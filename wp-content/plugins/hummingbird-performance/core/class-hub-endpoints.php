<?php
/**
 * Hummingbird - Hub endpoints: Hub_Endpoints class
 *
...
class Hub_Endpoints {
	...
	public function action_get( $params, $action ) {
		 ...
		 * Caching
		 */
		$status = Utils::get_status( 'caching' );
	...
	public function action_schedule( $params, $action ) {
		...
		if ( 'performance' === $module || 'reports' === $module ) {
			...
			// Reschedule.
			$next_scan_time = Pro\Modules\Reports::get_scheduled_time( $module, false );
	...
	public function action_purge_all_cache( $params, $action ) {
		WP_Hummingbird::flush_cache( true, false );
		wp_send_json_success();
	}
