...
class Gzip extends Page {
	...
	public function on_load() {
		$this->htaccess_written = Module_Server::is_htaccess_written( 'gzip' );
		$this->status           = Utils::get_status( 'gzip' );
		...
		if ( isset( $_GET['enable'] ) ) { // Input var ok.
			...
			$result = Module_Server::save_htaccess( 'gzip' );
			if ( $result ) {
				$redirect = true;
				$enabled  = true;

				// Clear saved status.
				$gzip_module = Utils::get_module( 'gzip' );
		...
		if ( isset( $_GET['disable'] ) ) { // Input var ok.
			...
			$result = Module_Server::unsave_htaccess( 'gzip' );
			if ( $result ) {
				$redirect = true;
				$disabled = true;
	...
	protected function render_inner_content() {
		...
		$server_type = array_search( $server_name, Module_Server::get_servers(), true );
		$this->view(
			$this->slug . '-page',
			array(
	...
	public function gzip_summary_metabox() {
		$inactive_types = Utils::get_number_of_issues( 'gzip', $this->status );

		$this->view(
			'gzip/summary-meta-box',
			array(
				'status'           => $this->status,
				'external_problem' => Utils::get_module( 'gzip' )->check_gzip_issues(),
	...
	public function gzip_summary_metabox_header() {
		...
		$this->view(
			...
			array(
				'issues'       => Utils::get_number_of_issues( 'gzip', $this->status ),
	...
	public function gzip_configure_metabox() {
		...
		$this->view(
			...
			array(
				...
				'gzip_server_type'   => Module_Server::get_server_type(),
				'full_enabled'       => $full_enabled,
				'recheck_url'        => $recheck_url,
