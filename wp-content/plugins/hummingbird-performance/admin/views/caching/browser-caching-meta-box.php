	...
	<?php if ( $htaccess_issue ) : ?>
		<div class="wphb-caching-error sui-notice sui-notice-error">
			...
			printf(
				...
				esc_url( \Hummingbird\Core\Utils::get_link( 'support' ) )
			);
			?>
		</div>
	<?php endif; ?>
...
<?php if ( $show_cf_notice ) : ?>
	<div class="sui-box-settings-row sui-upsell-row cf-dash-notice sui-no-padding-top">
		<?php if ( ! \Hummingbird\Core\Utils::hide_wpmudev_branding() ) : ?>
		...
		<div class="<?php echo( \Hummingbird\Core\Utils::hide_wpmudev_branding() ? esc_attr( 'sui-notice' ) : esc_attr( 'sui-upsell-notice' ) ); ?>">
			<p>
				<?php
				printf(
				...
				<span>
					...
					<a href="<?php echo \Hummingbird\Core\Utils::hide_wpmudev_doc_link() ? '#' : 'https://premium.wpmudev.org/blog/cloudflare-review/'; ?>" target="_blank">
